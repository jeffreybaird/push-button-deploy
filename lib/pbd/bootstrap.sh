#!/usr/bin/env bash
#
# lib/pbd/bootstrap.sh — `pbd bootstrap`: push-button stand-up for an app on
# DigitalOcean. Sourced by bin/pbd, which calls pbd_cmd_bootstrap.
#
#   pbd check [app_dir]                # verify prerequisites, exit non-zero on first gap
#   pbd bootstrap [app_dir]            # provision + wire + deploy (default app_dir: .)
#   pbd bootstrap --host <host_app_dir> [app_dir]
#                                      # deploy onto a droplet that ALREADY serves
#                                      # <host_app_dir>'s app (a "tenant")
#   pbd bootstrap --cli <lang> [app_dir]
#                                      # build a command-line program instead —
#                                      # repo + CI, no droplet and no credentials
#   pbd bootstrap --no-droplet [app_dir]
#                                      # build a reusable package the same way
#   pbd help bootstrap                 # the full type + language list
#
...
# The transcript goes in the STATE directory, not next to the templates: an
# installed pbd has no business writing into its own install prefix, and one
# log per machine beats one per checkout. `pbd config` prints the path.
LOG_FILE="$PBD_STATE_DIR/bootstrap.log"
: > "$LOG_FILE"
...
fail()  { printf '\033[31mpbd bootstrap: %s\033[0m\n' "$*" >&2; printf 'FAIL [%s] %s\n' "$(ts)" "$*" >> "$LOG_FILE"; exit 1; }
...
set -E
trap 'printf "\033[31mpbd bootstrap: unexpected failure at line %s (running: %s)\033[0m\n" "$LINENO" "$BASH_COMMAND" >&2' ERR
trap 'rc=$?; if [ "$rc" -ne 0 ]; then
        printf "\033[31m==> pbd bootstrap FAILED (exit %s) at step %s/%s\033[0m — transcript: %s\n" "$rc" "$STEP" "$TOTAL_STEPS" "$LOG_FILE" >&2
      else
        printf "==> run OK\n" >> "$LOG_FILE"
      fi' EXIT

# CONFIGURATION is already loaded: bin/pbd sources lib/pbd/common.sh and calls
# pbd_load_env before this file, so the env file (searched for at $PBD_ENV_FILE,
# then $XDG_CONFIG_HOME/pbd/env, then $PBD_ROOT/.env) and the calling shell's
# overrides are both in the environment by now. `pbd config` says which file.
# Everything below therefore reads plain variables and nothing sources anything.
...
. "$PBD_ROOT/scripts/app-types.sh"

# shellcheck source=../../scripts/provider.sh
. "$PBD_ROOT/scripts/provider.sh"
...
TPL_PERS_DIR="$PBD_ROOT/infra-persistent"
TPL_APP_TF_DIR="$PBD_ROOT/infra-app"
TPL_STATE_TF_DIR="$PBD_ROOT/infra-state"
TPL_TENANT_TF_DIR="$PBD_ROOT/infra-tenant"
...
ensure_app() {
  ...
  "$PBD_ROOT/scripts/inject-skill-docs.sh" "$APP_DIR"
}
...
sync_infra() {
  if is_tenant; then
    ...
    # takes the host down.
    "$PBD_ROOT/scripts/sync-infra.sh" --tenant "$APP_DIR"
    [ -d "$TENANT_TF_DIR" ] || fail "sync-infra did not create $TENANT_TF_DIR"
    return 0
  fi
  "$PBD_ROOT/scripts/sync-infra.sh" "$APP_DIR"
...
wait_droplet_ready() {
  ...
  if is_tenant; then
    ...
    fail "cannot reach the shared droplet at $APP_IP over SSH.
       ...
       SSH_CIDRS='[\"<host-operator-ip>/32\",\"$(curl -fsS https://api.ipify.org 2>/dev/null || echo x.x.x.x)/32\"]' pbd bootstrap $HOST_APP_DIR"
  fi
...
copy_deploy_files() {
  ...
  cp "$PBD_ROOT/deploy/Caddyfile" \
     "$PBD_ROOT/deploy/edge-compose.yaml" \
     "$PBD_ROOT/deploy/edge.sh" \
     "$APP_DIR/deploy/"
  ...
  if is_static; then
    ...
    cp "$PBD_ROOT/deploy/site.static.caddy.tmpl" "$APP_DIR/deploy/site.caddy.tmpl"
    cp "$PBD_ROOT/deploy/publish.sh"             "$APP_DIR/deploy/publish.sh"
    rm -f "$APP_DIR/deploy/swap.sh"
  else
    # Caddy reverse-proxies to the live color; publishing is the blue/green swap.
    cp "$PBD_ROOT/deploy/site.caddy.tmpl" "$APP_DIR/deploy/site.caddy.tmpl"
    cp "$PBD_ROOT/deploy/swap.sh"         "$APP_DIR/deploy/swap.sh"
    ...
    cp "$PBD_ROOT/deploy/staging-down.sh" "$APP_DIR/deploy/staging-down.sh"
  fi
}
...
copy_staging_files() {
  wants_staging || return 0
  cp "$PBD_ROOT/deploy/compose.staging.yaml"   "$APP_DIR/deploy/compose.staging.yaml"
  cp "$PBD_ROOT/deploy/litestream.staging.yml" "$APP_DIR/deploy/litestream.staging.yml"
}
...
copy_workflows() { # $1 template dir (repo-relative), $2 dest dir (app-relative)
  ...
  for pair in $(framework_workflows "$APP_TYPE" "$FRAMEWORK"); do
    ...
    cp "$PBD_ROOT/$1/$src" "$APP_DIR/$2/$dst"
  done
}
...
prep_app() {
  ...
  if is_sinatra; then
    ...
    cp "$PBD_ROOT/app/Dockerfile.ruby"    "$APP_DIR/Dockerfile"
    cp "$PBD_ROOT/app/.dockerignore.ruby" "$APP_DIR/.dockerignore"
    pin_ruby
    mkdir -p "$APP_DIR/$wf_dir" "$APP_DIR/deploy"
    copy_workflows "$tpl_wf_dir" "$wf_dir"
    ...
    cp "$PBD_ROOT/deploy/compose.sinatra.yaml" "$APP_DIR/deploy/compose.yaml"
    cp "$PBD_ROOT/deploy/litestream.yml"       "$APP_DIR/deploy/litestream.yml"
    copy_staging_files
    return 0
  fi
  ...
  cp "$PBD_ROOT/app/Dockerfile"     "$APP_DIR/Dockerfile"
  pin_toolchain
  cp "$PBD_ROOT/app/.dockerignore"  "$APP_DIR/.dockerignore"
  mkdir -p "$APP_DIR/$wf_dir" "$APP_DIR/deploy"
  copy_workflows "$tpl_wf_dir" "$wf_dir"
  ...
  if is_sqlite; then
    ...
    cp "$PBD_ROOT/deploy/compose.sqlite.yaml" "$APP_DIR/deploy/compose.yaml"
    cp "$PBD_ROOT/deploy/litestream.yml"      "$APP_DIR/deploy/litestream.yml"
    copy_staging_files
  else
    cp "$PBD_ROOT/deploy/compose.yaml" "$APP_DIR/deploy/"
    "$PBD_ROOT/scripts/ensure-db-tls.sh" "$APP_DIR"
  fi
  "$PBD_ROOT/scripts/ensure-release-task.sh" "$APP_DIR"
}
...
diagnose() {
  {
    ...
    printf '\nnext: %s; after a fix, re-run pbd bootstrap (idempotent)\n' "$(ci_watch_hint)"
  } >&2
  exit 1
}
...
# ---- the command ---------------------------------------------------------------
# abs_dir() — an absolute path for a directory that may not exist yet — comes
# from lib/pbd/common.sh, which every command shares.

pbd_bootstrap_usage() {
  cat <<EOF
pbd bootstrap — stand up an app, its repo and its pipeline.

  pbd bootstrap [options] [app_dir]        app_dir defaults to .
...
# The command bin/pbd dispatches to. Everything above is the machinery it drives.
pbd_cmd_bootstrap() {
  local check=0 t arg val
  while [ $# -gt 0 ]; do
    case "$1" in
      --check) check=1; shift ;;
      --help|-h) pbd_bootstrap_usage; exit 0 ;;
      # The language axis, spelled on its own. `--cli ruby` and `--cli=ruby`
      # below reach the same variable.
      --lang|--language)
        [ -n "${2:-}" ] || die_usage pbd_bootstrap_usage "--lang needs a language"
      ...
      --host)
        [ -n "${2:-}" ] || die_usage pbd_bootstrap_usage "--host needs the host app's directory"
      ...
      --*)
        ...
        [ -n "$t" ] || die_usage pbd_bootstrap_usage "unknown argument: $1"
      ...
      -*) die_usage pbd_bootstrap_usage "unknown argument: $1" ;;
      *)  break ;;
    esac
  done
