#!/usr/bin/env bash
#
# lib/pbd/gitea-bootstrap.sh — `pbd gitea bootstrap`: stand up a self-hosted
# Gitea instance + Actions runner, so GIT_PROVIDER=gitea has something real to
# talk to. Sourced by bin/pbd, which calls pbd_cmd_gitea_bootstrap.
#
#   pbd gitea bootstrap --check    # verify prerequisites, exit non-zero on first gap
...
#
# Reuses the SAME DigitalOcean/DNSimple/Spaces credentials `pbd bootstrap`
# already needs (one env file works for both) plus a handful of Gitea-specific
...
LOG_FILE="$PBD_STATE_DIR/gitea-bootstrap.log"
: > "$LOG_FILE"

ts()    { date +%H:%M:%S; }
log()   { printf '\033[32m==>\033[0m [%s] %s\n' "$(ts)" "$*"; printf '==> [%s] %s\n' "$(ts)" "$*" >> "$LOG_FILE"; }
warn()  { printf '\033[33m==> WARN\033[0m [%s] %s\n' "$(ts)" "$*" >&2; printf 'WARN [%s] %s\n' "$(ts)" "$*" >> "$LOG_FILE"; }
fail()  { printf '\033[31mpbd gitea bootstrap: %s\033[0m\n' "$*" >&2; printf 'FAIL [%s] %s\n' "$(ts)" "$*" >> "$LOG_FILE"; exit 1; }
...
trap 'rc=$?; if [ "$rc" -ne 0 ]; then
        printf "\033[31m==> pbd gitea bootstrap FAILED (exit %s) at step %s/%s\033[0m — transcript: %s\n" "$rc" "$STEP" "$TOTAL_STEPS" "$LOG_FILE" >&2
      else
        printf "==> run OK\n" >> "$LOG_FILE"
      fi' EXIT
...
# ---- config resolution (after the env file, so it can set any of these) -------
...
# still works.
# They live in the STATE directory (`pbd config` prints it), not beside the
# templates — an installed pbd must not write secrets into its own prefix. A
# checkout that cached them the old way has them moved across on first run
# rather than minting a second admin account.
TOKEN_CACHE="$(pbd_state_file gitea-admin-token "$PBD_ROOT/.gitea-admin-token")"
PASSWORD_CACHE="$(pbd_state_file gitea-admin-password "$PBD_ROOT/.gitea-admin-password")"

# scripts/tfstate.sh reads the generic STATE_BUCKET/SPACES_REGION names — but
# this env file is SHARED with `pbd bootstrap`, whose users may have set
# STATE_BUCKET there as a per-APP override. Never let that leak into Gitea's own bucket
...
# Same fallback binaries as `pbd bootstrap`; shares the state-bucket
...
copy_gitea_files() {
  ...
  remote_scp "$PBD_ROOT/gitea-host/docker-compose.yaml" "$PBD_ROOT/gitea-host/Caddyfile" \
    "$PBD_ROOT/gitea-host/act_runner-config.yaml" \
...
==> Gitea is set up. Add these to the env file you run `pbd bootstrap` with:
...
you ever replace this droplet, re-run `pbd bootstrap` for each app to refresh it.
EOF
}
...
pbd_gitea_bootstrap_usage() {
  cat <<EOF
pbd gitea bootstrap — stand up a self-hosted Gitea instance and its Actions
runner, on their own infrastructure (infra-gitea/).

  pbd gitea bootstrap [options]        takes no positional arguments

Options:
  --check              verify prerequisites and exit; provisions nothing
  --replace-droplet    recreate the droplet instead of updating it in place.
                       Needed to DOWNSIZE: DigitalOcean refuses a resize onto a
                       plan with a smaller disk even with resize_disk = false.
                       Keeps the data volume, the reserved IP and the DNS
                       record; rebuilds only Docker and the images
  -h, --help           this message

Reuses the same DigitalOcean/DNSimple/Spaces credentials \`pbd bootstrap\` needs,
plus GITEA_ADMIN_EMAIL (required) and the optional GITEA_* settings in
.env.example. When it finishes it prints the GIT_PROVIDER=gitea block to add to
your env file.
EOF
}

# The command bin/pbd dispatches to.
pbd_cmd_gitea_bootstrap() {
  local check=0
  while [ $# -gt 0 ]; do
    case "$1" in
      --check) check=1; shift ;;
      --replace-droplet) REPLACE_DROPLET=1; shift ;;
      -h|--help) pbd_gitea_bootstrap_usage; exit 0 ;;
      -*) die_usage pbd_gitea_bootstrap_usage "unknown argument: $1" ;;
      *)  die_usage pbd_gitea_bootstrap_usage "unknown argument: $1 (pbd gitea bootstrap takes no positional args)" ;;
    esac
  done
