#!/usr/bin/env bash
#
# lib/pbd/teardown.sh — `pbd teardown`: destroy everything `pbd bootstrap`
# created, in reverse order. Sourced by bin/pbd, which calls pbd_cmd_teardown.
#
#
...
#
#   pbd teardown [--yes] [--delete-repo] [app_dir]
#
...
#                  to carry delete rights — see GIT_PROVIDER in .env.example)
...
# `pbd bootstrap` applied. Apps bootstrapped before infra/ existed fall back to
...
#
# Requires the same environment as `pbd bootstrap` — same env file, same rules.
#
...
set -E
trap 'printf "\033[31mpbd teardown: unexpected failure at line %s (running: %s)\033[0m\n" "$LINENO" "$BASH_COMMAND" >&2' ERR

# ---- helpers (mirror bootstrap.sh) ---------------------------------------------
fail() { printf 'pbd teardown: %s\n' "$*" >&2; exit 1; }
have() { command -v "$1" >/dev/null 2>&1; }
log()  { printf '\033[31m==>\033[0m [%s] %s\n' "$(date +%H:%M:%S)" "$*"; }

# CONFIGURATION is already loaded — bin/pbd sources lib/pbd/common.sh and calls
# pbd_load_env before this file, under the same precedence rule bootstrap runs
# with: THE CALLING SHELL WINS. A teardown that silently used the env file's
# DNS_ZONE instead of the one the operator named would destroy records in the
# wrong zone.

# Same provider abstraction bootstrap.sh uses (GIT_PROVIDER, is_github/
# is_gitea, repo_delete, ...) — sourced here rather than inherited, since only
# one command module is ever loaded per run. Needs fail()/log()/have(), already
# defined above.
...
pbd_teardown_usage() {
  cat <<EOF
pbd teardown — destroy what \`pbd bootstrap\` created, in reverse order.

  pbd teardown [options] [app_dir]      app_dir defaults to .

Options:
  --yes            skip the type-the-project-name confirmation
  --delete-repo    also delete the code-host repo. GitHub needs
                   \`gh auth refresh -s delete_repo\` first; Gitea needs a
                   GITEA_TOKEN carrying delete rights
  -h, --help       this message

Destroyed, in this order: the droplet and its firewall, then the persistent root
— THE DATABASE AND ALL ITS DATA, the VPC, the reserved IP and the DNS record —
then the app's repository in the container registry, then the Terraform state
bucket. A droplet-free app (a CLI, a library) owns none of that, so all this
offers it is --delete-repo. A tenant takes a much smaller path: its own DNS
record, its own stack on the host's droplet, and its route out of the shared
Caddy. Nothing that belongs to the host is touched.

NOT touched, ever: the container registry itself, the SSH key in DigitalOcean,
the DNSimple zone, and the local app directory.
EOF
}

# ---- the command ---------------------------------------------------------------
# Everything from here to the end of the file is the teardown procedure, in the
# order it destroys things, wrapped in the function bin/pbd calls. The body is
# deliberately left at its original indentation: re-indenting 350 lines to gain
# one level would bury every real change in this file and break the heredoc in
# backend_init below.
pbd_cmd_teardown() {
...
while [ $# -gt 0 ]; do
  case "$1" in
    ...
    -h|--help)     pbd_teardown_usage; exit 0 ;;
    -*)            die_usage pbd_teardown_usage "unknown argument: $1" ;;
    *)             APP_DIR="$1" ;;
  esac
  shift
done
...
# What the app IS, as `pbd bootstrap` recorded it. Apps created before .app-type
...
if [ "$NO_INFRA" = 1 ]; then
  ...
  PERS_DIR="$PBD_ROOT/infra-persistent"
  APP_TF_DIR="$PBD_ROOT/infra-app"
...
if [ "$NO_INFRA" = 1 ]; then
  if [ "$DELETE_REPO" != 1 ]; then
    ...
    log "the only thing `pbd bootstrap` created is the code-host repo; delete it with --delete-repo (the local directory is never touched)."
    exit 0
  fi
...
trap 'rc=$?; for f in $OVERRIDES; do rm -f "$f"; done
      [ "$rc" -ne 0 ] && printf "\033[31m==> pbd teardown FAILED (exit %s) — fix and re-run; already-destroyed resources are skipped\033[0m\n" "$rc" >&2
      exit "$rc"' EXIT
